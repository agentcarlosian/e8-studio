...
For E₈, the diagram has one branch. In this studio's convention the chain is α₁—α₂—α₃—α₄—α₅—α₆—α₇, with α₈ attached to α₃. All eight roots have squared length 2, and adjacent roots have inner product −1.`,
  ...
  dynkin: {
    ...
    body: `The Dynkin diagram of a simple Lie algebra records its Cartan data: nodes are simple roots, and bonds encode their relative lengths and angles. The diagram of E₈ is:
α₁—α₂—α₃—α₄—α₅—α₆—α₇ with α₃—α₈ (one branch, length 1 from α₃).

For the exceptional part of the McKay correspondence, begin with the orientation-preserving tetrahedral, octahedral, or icosahedral rotation group and lift it through SU(2) → SO(3). The resulting binary polyhedral groups produce the affine diagrams Ẽ₆, Ẽ₇, and Ẽ₈ through a tensor-product graph. Removing the affine node leaves the corresponding finite E₆, E₇, or E₈ diagram.

Cube/octahedron and dodecahedron/icosahedron share rotation groups because they are dual pairs. The Studio uses those correspondences as navigation and highlighting context; it does not identify a solid's vertices one-to-one with E₆, E₇, or E₈ roots.`,
  },
...
The octonions are non-associative — (a·b)·c ≠ a·(b·c) in general. Their unusual algebra participates in important constructions of exceptional groups. It is more accurate to describe that as a rich relationship than as a single causal explanation for E₈.`,
  ...
  quaternions_600cell: {
    ...
    body: `The 120 vertices of the 600-cell can be identified with the 120 unit quaternions in the binary icosahedral group. A spatial rotation through angle θ about the unit axis v̂ is represented by q = (cos(θ/2), sin(θ/2) · v̂), with q and −q mapping to the same rotation in SO(3).

The binary icosahedral group has nine conjugacy classes, with sizes 1+12+20+12+30+12+20+12+1 = 120. Quaternion angles on the 3-sphere and physical SO(3) rotation angles differ by the double-cover convention, so the Studio does not label them as interchangeable quantities.
...
(dim 𝔰𝔬(16) = 16·15/2 = 120; a half-spinor of Spin(2n) has dimension 2ⁿ⁻¹, here 2⁷ = 128.) This decomposition is useful when studying how E₈ contains Spin(16)-related structure. The separate heterotic-string result selects E₈ × E₈ as one of two consistent ten-dimensional gauge choices; the dimension split alone is not the reason for that selection.`,
  },
...
Coxeter systems generalize reflection-generated symmetry to Euclidean, spherical, and hyperbolic settings in many dimensions. Crystallographic reflection groups lead to root systems and finite Weyl groups; E₈ has a finite Euclidean Weyl group acting in eight dimensions. Lorentzian reflection groups instead preserve an indefinite quadratic form and belong to a different classification.
...
All four share the full icosahedral symmetry group H₃ (order 120). The great icosahedron and great stellated dodecahedron are duals of each other, as are the small stellated dodecahedron and great dodecahedron. Select any of them in this view to see their vertices — taken straight from the icosahedron and dodecahedron — connected by the "great" edges that make them stars.`,
  ...
  rotation_planes_4d: {
    ...
    body: `In 3D, a rotation is described by an axis and an angle. A simple rotation in 4D fixes a two-dimensional plane and rotates its orthogonal plane. More generally, every orientation-preserving 4D rotation can be put into a form that rotates two orthogonal planes by two angles; when both angles are nonzero it is called a double rotation. The Lie algebra of SO(4) has six coordinate-plane generators: XY, XZ, XW, YZ, YW, ZW.

This is why 4D rotation feels so alien. You can rotate in the XY plane and the ZW plane simultaneously and independently. If the two angular speeds are incommensurate, a point's path need not close.
