...
with sync_playwright() as p:
    ...
    page.evaluate("window.__app.params.pickedRoot = 42")
    time.sleep(3)
    print(f'After pickedRoot=42 (no refresh): {len(page_errors)} errors')

    page.evaluate("window.__app.refreshPanel()")
    time.sleep(3)
    print(f'After refreshPanel: {len(page_errors)} errors')
    if page_errors:
        print(f'  first: {page_errors[0][:300]}')
        print(f'  unique: {set(page_errors[:5])}')
