...
def main() -> int:
    ...
    try:
        with sync_playwright() as p:
            ...
            page.wait_for_function("before => window.__mobileApp.getMetrics().autoModelSwitchCount > before && window.__mobileApp.getState().modelMode === 'e8_2d'", arg=dynkin_auto_before["autoModelSwitchCount"], timeout=4000)
            dynkin_auto_after = page.evaluate("() => window.__mobileApp.getMetrics()")
            check("Auto model sequence skips manual-only Dynkin diagrams", dynkin_auto_after["lastAutoModelTarget"]["modelMode"] == "e8_2d" and dynkin_auto_after["lastModelMode"] == "e8_2d", str(dynkin_auto_after))
            ...
            check("all FX keep GPU chords on Ripple's direct full-strength layer", all(item["e8ChordRenderer"] == "webgl-lines" and item["e8ChordGpuDrawCalls"] == 1 and item["chordActive"] and not item["chordComposited"] for item in fx_rendering), str(fx_rendering))
            ...
            chord_visibility = page.evaluate("""() => {
                ...
                const snapshot = () => {
                    ...
                    return {
                        ...
                        colorMix: stats.e8ChordFxColorMix,
                        direct: !document.getElementById('mobile-e8-chord-canvas').classList.contains('fx-composited'),
            ...
            check("deep composited FX and native Ripple both retain direct saturated GPU chords", chord_visibility["deepFx"]["direct"] and chord_visibility["deepRipple"]["direct"] and chord_visibility["deepFx"]["colorMix"] >= 0.8 and chord_visibility["deepRipple"]["colorMix"] == 1 and chord_visibility["deepRipple"]["alphaBoost"] >= 2.99, str(chord_visibility))
            ...
            page.evaluate("() => window.__mobileApp.setZoom(3.15)")
            page.get_by_role("button", name="In", exact=True).click()
            e8_zoom_past_old_cap = page.evaluate("() => ({ state: window.__mobileApp.getState(), sliderMax: Number(document.getElementById('camera-zoom').max), viewOutput: document.getElementById('zoom-output').textContent, motionOutput: document.getElementById('camera-zoom-output').textContent })")
            check("E8 Coxeter zoom passes the former 320% cap", e8_zoom_past_old_cap["state"]["zoom"] > 3.2 and e8_zoom_past_old_cap["sliderMax"] == 25 and e8_zoom_past_old_cap["viewOutput"] == e8_zoom_past_old_cap["motionOutput"], str(e8_zoom_past_old_cap))
            page.get_by_role("button", name="Motion", exact=True).click()
            page.locator("#camera-zoom").fill("25")
            e8_max_zoom = page.evaluate("() => ({ state: window.__mobileApp.getState(), viewOutput: document.getElementById('zoom-output').textContent, motionOutput: document.getElementById('camera-zoom-output').textContent })")
            check("E8 Coxeter close inspection reaches 2500%", e8_max_zoom["state"]["zoom"] == 25 and e8_max_zoom["viewOutput"] == "2500%" and e8_max_zoom["motionOutput"] == "2500%", str(e8_max_zoom))
            page.get_by_role("button", name="View", exact=True).click()
            safe_model_zoom = page.evaluate("() => { window.__mobileApp.setState({ modelMode: 'platonic', shape: 'icosahedron', zoom: 8 }); return { state: window.__mobileApp.getState(), sliderMax: Number(document.getElementById('camera-zoom').max) }; }")
            check("3D models retain their safe zoom ceiling", safe_model_zoom["state"]["zoom"] == 3.2 and safe_model_zoom["sliderMax"] == 3.2, str(safe_model_zoom))
            page.evaluate("() => window.__mobileApp.setState({ modelMode: 'e8_2d', zoom: 1, selectedRoot: 43 })")
