...
def main() -> int:
    ...
    try:
        with sync_playwright() as p:
            ...
            page.evaluate("() => { window.__mobileApp.setState({ quality: 'smooth', rotation: 1.1, cameraTilt: 0.8, cameraPath: 'dive', autoRotate: true, e8MorphT: 0.72, zoom: 2.7, panX: 135, panY: -95 }); window.__mobileApp.openSettings('view'); }")
            page.locator('[data-action="reset-view"]').click()
            page.evaluate("() => window.__mobileApp.forceRender()")
            sdf_reset = page.evaluate("() => ({ state: window.__mobileApp.getState(), metrics: window.__mobileApp.getMetrics() })")
            check("SDF Reset restores every camera dimension and a visible baseline", abs(sdf_reset["state"]["rotation"]) < 0.001 and abs(sdf_reset["state"]["cameraTilt"] - 0.28) < 0.001 and sdf_reset["state"]["cameraPath"] == "manual" and not sdf_reset["state"]["autoRotate"] and sdf_reset["state"]["e8MorphT"] == 0 and sdf_reset["state"]["zoom"] == 1 and sdf_reset["state"]["panX"] == 0 and sdf_reset["state"]["panY"] == 0 and sdf_reset["metrics"]["lastRenderAllFrame"]["withinView"] and sdf_reset["metrics"]["lastInteractionType"] == "reset-view", str(sdf_reset))
            page.evaluate("() => { window.__mobileApp.setState({ rotation: 0.9, cameraTilt: 0.65, cameraPath: 'dive', autoRotate: true, e8MorphT: 0.9, zoom: 2.9, panX: -160, panY: 120 }); window.__mobileApp.forceRender(); }")
